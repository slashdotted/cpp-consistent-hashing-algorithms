...
DxHash::DxHash(uint32_t n_working): num_working(n_working), size(1), nodes(){
    ...
    nodes.resize(size, false);

    for(uint32_t i = 0; i < num_working; ++i){
        nodes[i] = true;
    }
}
...
void DxHash::resize(){
    ...
    if(new_size < size) {
        uint32_t num_insertion = std::count(nodes.begin() + new_size, nodes.end(), true);
        nodes.resize(new_size, false);
        for(uint32_t i = 0; i < new_size && num_insertion > 0; ++i){
            if(nodes[i] == false){
                nodes[i] = true;
                num_insertion--;
            }

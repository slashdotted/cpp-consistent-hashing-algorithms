...
public:
   ...
   */
  uint32_t bArraySize() const noexcept;
 ...
 */
template<template<typename...> class MementoMap, typename... Args>
MementoEngine<MementoMap, Args...>::MementoEngine(uint32_t, uint32_t size)
    : m_lastRemoved{size}
    , m_bArraySize{size}
{}
 ...
 */
template <template <typename...> class MementoMap, typename... Args>
uint32_t MementoEngine<MementoMap, Args...>::addBucket() noexcept {
...
template <template <typename...> class MementoMap, typename... Args>
uint32_t
MementoEngine<MementoMap, Args...>::removeBucket(uint32_t bucket) noexcept {
  /*
 ...
 */
template <template <typename...> class MementoMap, typename... Args>
inline uint32_t MementoEngine<MementoMap, Args...>::size() const noexcept {
  return m_bArraySize - m_memento.size();
}
...
template <template <typename...> class MementoMap, typename... Args>
inline uint32_t
MementoEngine<MementoMap, Args...>::bArraySize() const noexcept {
  return m_bArraySize;
}

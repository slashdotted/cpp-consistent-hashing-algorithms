...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        YAML::Node yaml_config = yaml_file["monotonicity"][config];
        ...
        if (yaml_config) {
            ...
            if (algo == "anchor") {
                return bench<AnchorEngine>("AnchorEngine", "anchor_monotonicity.log", anchor_set, working_set, num_removals, num_keys);
            } else if (algo == "dx") {
                return bench<DxEngine>("DxEngine", "dx_monotonicity.log", anchor_set, working_set, num_removals, num_keys);
            } else if (algo == "jump") {
                return bench<JumpEngine>("JumpEngine", "jump_monotonicity.log", anchor_set, working_set, num_removals, num_keys);
            } else if (algo == "maglev") {
                return bench<MaglevEngine>("MaglevEngine", "maglev_monotonicity.log", anchor_set, working_set, num_removals, num_keys);
            } else if (algo == "memento") {
                return bench<MementoEngine<boost::unordered_flat_map>>("MementoEngine<boost::unordered_flat_map>", "memento_monotonicity.log", anchor_set, working_set, num_removals, num_keys);
            } else if (algo == "power") {
                return bench<PowerEngine>("PowerEngine", "power_monotonicity.log", anchor_set, working_set, num_removals, num_keys);
            } else if (algo == "ring") {
                return bench<RingEngine>("RingEngine", "ring_monotonicity.log", anchor_set, working_set, num_removals, num_keys);
            } else {
